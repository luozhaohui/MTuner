...
{
public:
    TreeItem(CaptureContext* _context,
             const rtm::StackTraceTree* _tree,
             TreeItem* _parent,
             const rtm::StackTraceTree* _root,
             int _depth);
    ~TreeItem();
...
{
    return _in1->m_tree->m_opCount[rtm::StackTraceTree::Alloc] <
           _in2->m_tree->m_opCount[rtm::StackTraceTree::Alloc];
}
...
{
    return _in1->m_tree->m_opCount[rtm::StackTraceTree::Free] <
           _in2->m_tree->m_opCount[rtm::StackTraceTree::Free];
}
...
{
    return _in1->m_tree->m_opCount[rtm::StackTraceTree::Realloc] <
           _in2->m_tree->m_opCount[rtm::StackTraceTree::Realloc];
}
...
TreeItem::TreeItem(CaptureContext* _context,
                   const rtm::StackTraceTree* _tree,
                   TreeItem* _parent,
                   const rtm::StackTraceTree* _root,
                   int _depth)
...
QVariant TreeItem::data(int _column) const
{
    if (m_parent == 0)
    {
        switch (_column)
        {
            ...
            case Header::PeakUsage:
                return QObject::tr("Peak Usage");
            case Header::Allocs:
                return QObject::tr("Allocations");
            case Header::Frees:
                return QObject::tr("Frees");
            case Header::Reallocs:
                return QObject::tr("Reallocations");
            case Header::File:
    ...
    else
    {
        if (!m_resolved)
        {
            ...
            m_context->resolveStackFrame(m_tree->m_stackTraceList->m_frames[m_tree->m_stackTraceList->m_numFrames - m_depth],
                                         frame);
        ...
        switch (_column)
        {
            ...
            case Header::Module:
                return m_module;
            case Header::Usage:
                return m_tree->m_memUsage
                           ? ((float(m_tree->m_memUsage) * 100.0f) / float(m_root->m_memUsage))
                           : 0;
            case Header::PeakUsage:
                return m_tree->m_memUsagePeak
                           ? ((float(m_tree->m_memUsagePeak) * 100.0f) / float(m_root->m_memUsagePeak))
                           : 0;
            case Header::Allocs:
                return formatPercentageView(m_tree->m_opCount[rtm::StackTraceTree::Alloc],
                                            m_root->m_opCount[rtm::StackTraceTree::Alloc]);
            case Header::Frees:
                return formatPercentageView(m_tree->m_opCount[rtm::StackTraceTree::Free],
                                            m_root->m_opCount[rtm::StackTraceTree::Free]);
            case Header::Reallocs:
                return formatPercentageView(m_tree->m_opCount[rtm::StackTraceTree::Realloc],
                                            m_root->m_opCount[rtm::StackTraceTree::Realloc]);
            case Header::File:
                return m_file;
            case Header::Line:

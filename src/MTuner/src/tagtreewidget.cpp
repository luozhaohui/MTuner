...
QVariant TagTreeItem::data(int _column) const
{
    if (m_parent == NULL)
    {
        switch (_column)
        {
            ...
            case 4:
                return QObject::tr("Peak Overhead");
        };
    }
    else
    {
        ...
        {
            case 0:
                if (m_tree->m_name.size() == 0)
                    return QString(QObject::tr("Root"));
                else
                    return QString(m_tree->m_name.c_str());
            case 1:
                return locale.toString(qulonglong(m_tree->m_usage));
            case 2:
                return locale.toString(qulonglong(m_tree->m_usagePeak));
            case 3:
                return locale.toString(qulonglong(m_tree->m_overhead));
            case 4:
                return locale.toString(qulonglong(m_tree->m_overheadPeak));
        };
    }
    return "";
}

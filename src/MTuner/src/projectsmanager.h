...
struct Project
{
    ...
    bool operator!=(const Project& _other)
    {
        return ((m_executablePath != _other.m_executablePath) || (m_cmdArgs != _other.m_cmdArgs) ||
                (m_workingDir != _other.m_workingDir) || (m_environment != _other.m_environment) ||
...
public:
    ...
    void addProject(const Project& _project)
    {
        m_projects.push_back(_project);
        updateProjectList();
    }
    ...
    const Project& getProject(int _index) const
    {
        return m_projects[_index];
    }
    bool run(const QString& _executable,
             const QString& _cmd = QString(),
             const QString& _workingDir = QString(),
             const QStringList& _environment = QStringList(),
             bool _inheritEnv = true,
             int _allocator = 0,

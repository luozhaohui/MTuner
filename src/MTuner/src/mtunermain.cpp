...
bool handleInject(rtm::CommandLine& _cmdLine)
{
    ...
    if (profileWorkDir == NULL)
    {
        ...
        while ((workingDir[end] != '/') && (workingDir[end] != '\\'))
            --end;
        workingDir[end + 1] = '\0';
    }
    else
...
int handleCommandLine(int argc, char const* argv[])
{
...

    {
        ...
        {
            setupLoaderToolchain(&context,
                                 inFilePath,
                                 &gcc_setup,
                                 NULL,
                                 NULL,
                                 symSource ? QString(symSource) : QString(""),
                                 resolverCallBack);
            ...
            else if (!context.m_capture->saveGroupsLog(outFilePath, sorting, context.m_symbolResolver))
            {
...
int main(int argc, const char* argv[])
{
    ...
    if (info.exists() && !skipDiaRegister)
    {
        ...
        {
            QMessageBox regFail(QMessageBox::Warning,
                                QObject::tr("Failed to register DIA dll!"),
                                QObject::tr("Debug symbols may not be loaded correctly"));
        }
    }
#endif
    ...
    {
        QStringList list;
        list << "resources/fonts/Maven Pro Bold.otf"
             << "resources/fonts/Maven Pro Medium.otf"
             << "resources/fonts/Maven Pro Regular.otf";

        for (QStringList::const_iterator constIterator = list.constBegin(); constIterator != list.constEnd();
             ++constIterator)
        {

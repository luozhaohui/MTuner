    ...
    : QDialog(_parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint)
{
    ...
    m_toolchainCombo->setIconSize(
        QSize(m_toolchainCombo->size().width(), m_toolchainCombo->iconSize().height()));
}
...
rdebug::Toolchain GCCSetup::getToolchainInfo(rmem::ToolChain::Enum _toolchain, bool _64bit)
{
    ...
    if ((_toolchain == rmem::ToolChain::PS3_gcc) || (_toolchain == rmem::ToolChain::PS4_clang) ||
...
bool GCCSetup::resolveToolchain(Toolchain& _toolchain, bool _64bit, rdebug::Toolchain& _tc)
{
    ...
    append10 += QString(".exe");
    append11 += QString(".exe");
    append20 += QString(".exe");
    append21 += QString(".exe");
    append30 += QString(".exe");
    append31 += QString(".exe");
#endif
...
void GCCSetup::setLabels()
{
    ...
    else if (tc.m_toolchain == rmem::ToolChain::PS5_clang)
    {

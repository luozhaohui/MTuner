...
public:
    ...
    void setDisplayMode(DisplayMode::Enum _mode)
    {
        m_displayMode = _mode;
        parentResized();
    }
    void setHistogramType(HistogramType::Enum _type)
    {
        m_type = _type;
        parentResized();
    }
    void setPeaks(bool _show)
    {
        m_showPeaks = _show;
        parentResized();
    }
    ...
    void setHighlight(const QPoint& _point, int _binIndex)
    {
        m_highlight = _point;
        m_highlightBin = _binIndex;
        parentResized();
    }
    int getHighlightIndex() const
    {
        return m_highlightBin;
    }

    const QVector<HistogramToolTip>& getTooltips() const
    {
        return m_toolTips;
    }

...
void Histogram::paint(QPainter* _painter, const QStyleOptionGraphicsItem* _option, QWidget* _widget)
{
    ...
    for (int i = 0; i < numBins; ++i)
    {
        ...
        if (i == static_cast<int>(selectedBin))
            _painter->setPen(
                QPen(QColor(50 + 80, 150 + 80, 170 + 80), 1.0, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
        else
        ...
        _painter->restore();
        _painter->drawLine(QPoint(left + currPos - deltaW, bottom),
                           QPoint(left + currPos - deltaW, bottom + 3));
        currPos += delta;
        currSize <<= 1;
    }
                ...
                if (m_showPeaks)
                {
                    ...
                    if (binHeight != 0)
                    {
                        ...
                        m_toolTips.push_back(
                            HistogramToolTip(rect2, statsTypePeak + getTypeString(m_type, binVal), i));
                    }
                }
            ...
            for (int i = 0; i < numBins; ++i)
            {
                if (m_showPeaks)
                {
                    ...
                    if (binHeight != 0)
                    {
                        ...
                        _painter->setBrush(
                            boostColor(globalCol, shouldHighlight(rect1, m_highlight, m_highlightBin)));
                        _painter->drawRect(rect1);
                        m_toolTips.push_back(
                            HistogramToolTip(rect1, s_global + getTypeString(m_type, binVal), i));
                    }
                    ...
                    if (binHeight != 0)
                    {
                        ...
                        _painter->setBrush(
                            boostColor(globalColPeak, shouldHighlight(rect2, m_highlight, m_highlightBin)));
                        _painter->drawRect(rect2);
                        m_toolTips.push_back(
                            HistogramToolTip(rect2, s_globalPeak + getTypeString(m_type, binVal), i));
                    }
                    ...
                    if (binHeight != 0)
                    {
                        ...
                        _painter->setBrush(
                            boostColor(snapshotCol, shouldHighlight(rect3, m_highlight, m_highlightBin)));
                        _painter->drawRect(rect3);
                        m_toolTips.push_back(
                            HistogramToolTip(rect3, s_snapshot + getTypeString(m_type, binVal), i));
                    }
                    ...
                    if (binHeight != 0)
                    {
                        ...
                        _painter->setBrush(boostColor(snapshotColPeak,
                                                      shouldHighlight(rect4, m_highlight, m_highlightBin)));
                        _painter->drawRect(rect4);
                        m_toolTips.push_back(
                            HistogramToolTip(rect4, s_snapshotPeak + getTypeString(m_type, binVal), i));
                    }
                ...
                else
                {
                    ...
                    if (binHeight != 0)
                    {
                        QRect rect2(position, bottom - binHeight, thickness / 2, binHeight);
                        _painter->setBrush(
                            boostColor(snapshotCol, shouldHighlight(rect2, m_highlight, m_highlightBin)));
                        _painter->drawRect(rect2);
                        m_toolTips.push_back(
                            HistogramToolTip(rect2, s_snapshot + getTypeString(m_type, binVal), i));
                    }

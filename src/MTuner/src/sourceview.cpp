...
void SourceView::changeEvent(QEvent* _event)
{
    ...
    switch (_event->type())
    {
        case QEvent::LanguageChange:
            if (m_tabWidthAction4)
                m_tabWidthAction4->setText(QObject::tr("Set tab width to 4"));
            if (m_tabWidthAction8)
                m_tabWidthAction8->setText(QObject::tr("Set tab width to 8"));
            if (m_openInEditorAction)
                m_openInEditorAction->setText(QObject::tr("Open in editor"));
            break;
        default:
            break;
    };
...
void SourceView::openInEditor()
{
    ...
    {
        int b = QMessageBox::question(this,
                                      tr("Failed to start editor!"),
                                      tr("Setup external editor now?"),

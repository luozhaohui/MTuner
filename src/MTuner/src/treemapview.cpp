...
static inline uint64_t getNodeValueByType(TreeMapNode& _tree, uint32_t _type)
{
    ...
    {
        case 0:
            return _tree.m_tree->m_memUsage;
        case 1:
            return _tree.m_tree->m_memUsagePeak;
        case 2:
            return _tree.m_tree->m_overhead;
        case 3:
            return _tree.m_tree->m_overheadPeak;
    };
    return 0;
}
    ...
    for (size_t i = 0; i < m_tree.size(); ++i)
        if (m_tree[i].m_rect.contains(scenePos))
            if (m_highlightNode != &m_tree[i])
            {
                ...
                QLocale locale;
                QString str =
                    QString("<pre>") + QStringColor(tr("Total size"), "ff42a6ba") +
                    QStringColor(locale.toString(qulonglong(m_highlightNode->m_size)), "ffffff33", false) +
                    QString("<br>") + QStringColor(tr("Operations"), "ff83cf67") +
                    locale.toString(
                        qulonglong(m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Alloc] +
                                   m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Realloc] +
                                   m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Free])) +
                    QString("<br><br>") + QStringColor(tr("  Allocs"), "ffffffff") +
                    locale.toString(qulonglong(m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Alloc])) +
                    QString("<br>") + QStringColor(tr("Reallocs"), "ffffffff") +
                    locale.toString(qulonglong(m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Realloc])) +
                    QString("<br>") + QStringColor(tr("   Frees"), "ffffffff") +
                    locale.toString(qulonglong(m_highlightNode->m_tree->m_opCount[rtm::StackTraceTree::Free])) +
                    QString("</pre>");
...
void TreeMapView::buildTree()
{
    ...
    const rtm::StackTraceTree& tree = filtered ? m_context->m_capture->getStackTraceTreeFiltered()
                                               : m_context->m_capture->getStackTraceTree();

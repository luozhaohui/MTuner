...
{
    return QString("<font color=\"#") + QString(_color) + QString("\">") + _string +
           (_addColon ? QString(":</font> ") : QString("</font> "));
}
...
void GraphWidget::setContext(CaptureContext* _context, BinLoaderView* _view)
{
    ...
    if (_context)
    {
        ...
        if ((m_context->m_capture->getMinTime() == m_context->m_capture->getSnapshotTimeMin()) &&
            ...
            m_select->setSelectRange(m_context->m_capture->getSnapshotTimeMin(),
                                     m_context->m_capture->getSnapshotTimeMax());
    }
    invalidateScene();
}
...
void GraphWidget::zoomOut(uint64_t _time)
{
    ...
    if ((newMinTime == (int64_t)m_context->m_capture->getMinTime()) &&
        (newMaxTime == (int64_t)m_context->m_capture->getMaxTime()))
...
void GraphWidget::updateToolTip(const QPoint& _position)
{
    ...
    if (rect().contains(_position) && m_LButtonDown && !m_inContextMenu)
    {
        ...
        QString ttip = sqs_PRE + sqs_1 + getTimeString(startTimeF) + sqs_BR + sqs_2 +
                       getTimeString(endTimeF) + sqs_BR + sqs_3 + getTimeString(endTimeF - startTimeF) +
                       sqs_BR + sqs_BR + sqs_4 + m_locale.toString(qulonglong(entry.m_usage)) + sqs_BR +
                       sqs_5 + m_locale.toString(qulonglong(entry.m_numLiveBlocks)) + QString("</pre>");

        m_toolTipLabel->setText(ttip);
        m_toolTipLabel->adjustSize();
    }
    else if (drawRect.contains(pt.x(), pt.y()) && !m_LButtonDown && !m_inContextMenu)
    {
        ...
        QString ttip = sqs_PRE + sqs_1 + timeStr + sqs_BR + sqs_2 +
                       m_locale.toString(qulonglong(entry.m_usage)) + sqs_BR + sqs_3 +
                       m_locale.toString(qulonglong(entry.m_numLiveBlocks)) + sqs_PRE + QString("\n");
...
void GraphWidget::mouseMovement(const QPoint& _position, Qt::MouseButtons _buttons)
{
    ...
    for (int i = 0; i < toolTips.size(); ++i)
    {
        if (toolTips[i].m_rect.contains(spt) && !m_inContextMenu)
        {
            ...
            text = text + QString("\n") + tr("Time") + QString(": ") +
                   getTimeString(m_context->m_capture->getFloatTime(toolTips[i].m_time));
            text = text + QString("\n") + tr("Thread") + QString(": 0x") +
                   QString::number(toolTips[i].m_threadID, 16);
...
void GraphWidget::mouseReleaseEvent(QMouseEvent* _event)
{
    ...
    if (_event->button() == Qt::LeftButton)
    {
        ...
        {
            m_context->m_capture->setSnapshot(m_context->m_capture->getMinTime(),

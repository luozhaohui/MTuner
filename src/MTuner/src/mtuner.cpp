...
void setupLoaderToolchain(CaptureContext* _context,
                          const QString& _file,
                          GCCSetup* _gccSetup,
                          QFileDialog* _fileDialog,
                          MTuner* _mtuner,
                          const QString& _symSource,
                          rdebug::module_load_cb _callBack)
{
    rdebug::Toolchain tc;
    ...
    if (_context->m_capture->getToolchain() != rmem::ToolChain::Win_MSVC)
    {
        ...
        {
            case rmem::ToolChain::Win_gcc:
                extensions = "Exe files (*.exe)";
                break;
            case rmem::ToolChain::Linux_gcc:
                break;
            case rmem::ToolChain::OSX_gcc:
                break;
            case rmem::ToolChain::PS5_clang:
                extensions = "Elf files (*.self)";
                break;
            case rmem::ToolChain::PS4_clang:
                extensions = "Elf files (*.elf)";
                break;
            case rmem::ToolChain::PS3_gcc:
                extensions = "Elf files (*.elf)";
                break;
            case rmem::ToolChain::PS3_snc:
                extensions = "Self files (*.self)";
                break;
            case rmem::ToolChain::Android_arm:
                extensions = "so files (*.so)";
                break;
            case rmem::ToolChain::Android_mips:
                extensions = "so files (*.so)";
                break;
            case rmem::ToolChain::Android_x86:
                extensions = "so files (*.so)";
                break;
            default:
                break;
        ...
        if (_fileDialog && !symSrcFound)
        {
            ...
            QString fileName =
                _fileDialog->getOpenFileName(_mtuner, QObject::tr("select symbol source"), dir, extensions);
        ...
        if (!_gccSetup->isConfigured(_context->m_capture->getToolchain(), _context->m_capture->is64bit()))
        {
            if (_fileDialog)
            {
                ...
                QMessageBox message(QMessageBox::Question,
                                    QObject::tr("Toolchain is not configured"),
                                    QObject::tr("Toolchain needs to be configured in order to resolve symbols.\nWould you like to configure the toolchain now?"),
                ...
                if (message.result() == QMessageBox::Yes)
                {
                    ...
                    QMessageBox::information(0,
                                             QObject::tr("Toolchain is not configured"),
                                             QObject::tr("No debug symbols can be resolved!"));
            }
            else
            {
                // cmd
...
void MTuner::changeEvent(QEvent* _event)
{
    ...
    switch (_event->type())
    {
        case QEvent::LanguageChange:
            ...
            if (m_graphDock)
                m_graphDock->setWindowTitle(tr("Memory timeline"));
            if (m_histogramDock)
                m_histogramDock->setWindowTitle(tr("Allocation histogram"));
            if (m_statsDock)
                m_statsDock->setWindowTitle(tr("Statistics"));
            if (m_tagTreeDock)
                m_tagTreeDock->setWindowTitle(tr("Memory tag tree"));
            if (m_stackAndSourceDock)
                m_stackAndSourceDock->setWindowTitle(tr("Stack trace"));
            if (m_heapsDock)
                m_heapsDock->setWindowTitle(tr("Heaps / Allocators"));
            if (m_modulesDock)
                m_modulesDock->setWindowTitle(tr("Modules"));

            // Ugly hack for a bug in Qt that incorrectly renders QAction items in menu if they have
            // both an icon and are checkable Seems to do the trick... for now.
...
void MTuner::openFile()
{
    ...
    QString fileName =
        m_fileDialog->getOpenFileName(this, tr("select a capture file"), getCaptureLocation(), "MTuner files (*.MTuner)");
...
void MTuner::setDockWindowIcon(DockWidget* _widget, const QString& _icon)
{
    ...
    ToolButtonHover* buttonToggleDock =
        new ToolButtonHover(QIcon(":/MTuner/resources/images/dock_detach.png"),
                            QIcon(":/MTuner/resources/images/dock_detach_hover.png"));
    ...
    ToolButtonHover* buttonCloseDock =
        new ToolButtonHover(QIcon(":/MTuner/resources/images/dock_close.png"),
                            QIcon(":/MTuner/resources/images/dock_close_hover.png"));
...
void MTuner::setupDockWindows()
{
    ...
    connect(m_centralWidget,
            ...
            m_stackAndSource,
            SLOT(setStackTrace(rtm::StackTrace**, int)));
...
void MTuner::setWidgetSources(CaptureContext* _context)
{
    ...
    if (binView)
    {
        ...
        connect(binView,
                SIGNAL(selectRange(uint64_t, uint64_t)),
                m_graph->getGraphWidget(),
                SLOT(selectFromTimes(uint64_t, uint64_t)));
...
void MTuner::readSettings()
{
    ...
    if (settings.contains("major"))
        major = (uint8_t)settings.value("major").toInt();
    if (settings.contains("minor"))
        minor = (uint8_t)settings.value("minor").toInt();
    if (settings.contains("detail"))
        detail = (uint8_t)settings.value("detail").toInt();
    settings.endGroup();
...
void MTuner::openFileFromPath(const QString& _file)
{
    ...
    if (_file.size() != 0)
    {
        ...
        else
        {
            ...
            QMessageBox info_dlg(QMessageBox::Information,
                                 tr("Failed to load file!"),
                                 tr("File may be corrupted, try to repeat the capture"),
                                 QMessageBox::Ok);
    ...
    if (_file.toLower().endsWith(".exe"))
    {
        if (m_capturePid)
        {
            QMessageBox message(QMessageBox::Information,
                                ...
                                tr("Cannot start a new capture while capture is in progress"));
            message.exec();
            return false;
        }
        else
